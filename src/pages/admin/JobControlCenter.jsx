...
import {
  Loader2, Car, User, Phone, Wrench, 
  AlertCircle, ShieldCheck, Clock, RefreshCw
} from "lucide-react";
    ...
    <div className="group bg-white rounded-3xl border border-slate-200 p-5 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all duration-300 flex flex-col justify-between">
      ...
      <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 border border-white shadow-sm">
            {job.assignedStaff?.name?.charAt(0) || "S"}
          </div>
          <div>
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`px-3 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-wider ring-1 ring-inset ${styles[status] || "bg-slate-50 text-slate-600 ring-slate-100"}`}>
      {status}
    </span>
  );
}
